-- require()'s internal package.searchpath calls fopen() directly, bypassing the
-- UTF8->ACP io.open/dofile patches Quarto's init.lua installs on Windows, so it
-- can't find modules under a non-ASCII (e.g. Korean) path. dofile() with an
-- absolute path does go through the patched loader, so use it first.
local function load_tinyyaml()
  if PANDOC_SCRIPT_FILE then
    local dir = pandoc.path.directory(PANDOC_SCRIPT_FILE)
    local ok, mod = pcall(dofile, pandoc.path.join({ dir, "resources", "tinyyaml.lua" }))
    if ok then return mod end
  end
  return require "resources/tinyyaml"
end
...
local function tree(root)
  ...
  function gather(path, list)
    -- isdir()'s os.rename trick can false-positive on Windows for files, so
    -- fall back to treating path as a file if list_directory then fails.
    if isdir(path) then
      local ok, items = pcall(pandoc.system.list_directory, path)
      if ok then
